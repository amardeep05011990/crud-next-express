...
@Component({
  selector: 'app-demouser-form',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './demouser-form.component.html'
})
export class DemouserFormComponent implements OnInit {
  @Input() item: any = null;
  ...
  onSubmit() {
    ...
    if (this.item?._id) {
      this.http.put(`${environment.apiBaseUrl}/api/demouser/${this.item._id}`, data).subscribe({
        next: () => {
          this.successMessage = 'Demouser updated successfully!';
          this.message.emit({ type: 'success', text: this.successMessage });
          this.formSaved.emit();
          this.resetForm();
        },
        error: () => {
          this.errorMessage = 'Failed to update Demouser.';
          this.isSubmitting = false;
        }
      });
    } else {
      this.http.post(`${environment.apiBaseUrl}/api/demouser`, data).subscribe({
        next: () => {
          this.successMessage = 'Demouser added successfully!';
          this.message.emit({ type: 'success', text: this.successMessage });
          this.formSaved.emit();
          this.resetForm();
        },
        error: () => {
          this.errorMessage = 'Failed to add Demouser.';
          this.isSubmitting = false;
        }
      });
    }
