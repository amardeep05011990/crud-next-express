...
const StudentsForm = () => {
  ...
  const loadData = async () => {
    const res = await fetch(`/api/students?page=${page + 1}&limit=${rowsPerPage}&search=${search}`);
          ...
          <TableBody>
            {data.map((item) => (
              <TableRow key={item._id}>
                <TableCell>{Array.isArray(item.city) ? item.city.join(', ') : item.city}</TableCell>
<TableCell>{Array.isArray(item.gender) ? item.gender.join(', ') : item.gender}</TableCell>
<TableCell>{Array.isArray(item.title) ? item.title.join(', ') : item.title}</TableCell>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle>{editId ? 'Edit' : 'Create'} Students</DialogTitle>
