    ...
    <div class="col-md-${grid} position-relative mb-3">
      <label for="${name}_search" class="form-label">${label}</label>
        <input type="text"
              ...
              (input)="onLookupSearch('${name}', ${name}searchBox.value)"
              [value]="getSelectedLabel('${name}')" />
      <ul class="list-group position-absolute w-100" *ngIf="${name}Options?.length > 0">
        <li
...
// Inside generated component class
// ${f.name}Options: { label: string; value: any }[] = [];
  [key: string]: any;
