...
export default function DisplayForm() {
  ...
  async function fetchCollectionData(formStructure) {
    ...
    for (const row of formStructure) {
      for (const column of row.columns) {
        ...
        }else if(column.source === "relation" && column.relationType == "Many-to-Many"){
              try {
                const response = await fetch(
                  `${apiUrl}/forms/${column.collection}/submissions`
                );
        ...
        console.log("column.relationType==>>",  column.relationType)
      }
    }
  ...
  const handleChange = (rowId, columnId, value, isCheckbox = false,  isMultiSelect = false) => {
    setFormValues((prev) => {
      ...
      if (isMultiSelect) {
        return { ...prev, [key]: Array.isArray(value) ? value : [value] }; // Ensure it's always an array
      }
          ...
          >
            <div style={{ display: "flex", gap: "20px" }}>
              {row.columns.map((column) => (
                ...
                {column.type === "Dropdown" && column.relationType === "Many-to-Many" && (
                  <>
                    <InputLabel id={`label-${row.id}-${column.id}`}>{column.label}</InputLabel>
                    <Select
                      labelId={`label-${row.id}-${column.id}`}
                      id={`select-${row.id}-${column.id}`}
                      multiple
                      value={formValues[`${row.id}-${column.id}`] || []}
                      onChange={(e) => handleChange(row.id, column.id, e.target.value, false, true)}
                      fullWidth
                      error={!!errors[`${row.id}-${column.id}`]}
                    >
                      {getCollectionOptions(column.collection).map((opt) => (
